  ...
  return (
    <div
      ...
      className={classNames(
        "group focus-ring relative flex cursor-pointer items-center gap-2.5 rounded-md border border-transparent px-2.5 py-2 transition",
        "hover:border-white/[0.07] hover:bg-white/[0.025]",
        active && "border-white/[0.14] bg-white/[0.045]"
      )}
    >
      {isCritical && (
        <span
          aria-hidden
          className="pointer-events-none absolute inset-y-1.5 left-0 w-[2px] rounded-r-full bg-rose-400/70"
        />
      )}
      <SentimentDot sentiment={item.sentiment} />
      <div className="min-w-0 flex-1">
        <div className="line-clamp-1 text-[12.5px] font-medium leading-snug tracking-tightish text-white/85 group-hover:text-white">
          {item.headline}
        </div>
      </div>
      <span
        className={classNames(
          "shrink-0 font-mono text-[10.5px] font-semibold num tabular-nums",
          impactTone
        )}
        title={`Impact ${impact.toFixed(1)} / 10`}
      >
        {impact.toFixed(1)}
      </span>
      {showTime && (
        <span className="hidden whitespace-nowrap font-mono text-[10px] num text-white/40 sm:inline">
      ...
      {item.newsUrl ? (
        <a
          ...
          className="focus-ring inline-flex h-[22px] w-[22px] items-center justify-center rounded-md border border-white/[0.06] bg-white/[0.02] text-white/50 transition hover:border-white/[0.18] hover:bg-white/[0.04] hover:text-white"
        >
  ...
  return (
    ...
    >
      <div className="flex items-baseline justify-between gap-2">
        <div className="truncate text-[11.5px] font-semibold tracking-tightish text-white">
          {breakdown.sectorName}
        </div>
        <div
          className="font-mono text-[11.5px] font-bold num"
          style={{ color: heatHex }}
        ...
        {total === 0 ? (
          ...
          segs.map((s) => (
            ...
            >
              ...
              <div className="font-mono text-[12.5px] font-semibold num text-white/90">
                {s.count}
              </div>
            </div>
          ))
      ...
      <div className="mt-1.5 text-right font-mono text-[9px] num text-white/40">
